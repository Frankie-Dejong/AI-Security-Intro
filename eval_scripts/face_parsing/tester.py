...
class Tester(object):
    ...
    def test(self):
        ...
        test_paths = make_dataset(self.test_image_path, self.test_size)
        ...
        for i in tqdm(range(batch_num)):
            ...
            for k in range(self.batch_size):
                cv2.imwrite(os.path.join(self.test_label_path, pathes[k].split('/')[-1].split('.')[0]+'.png'), labels_predict_plain[k])
                save_image(labels_predict_color[k], os.path.join(self.test_color_label_path, pathes[k].split('/')[-1].split('.')[0]+'.png'))
